...
function generateInputs() {
  ...
  // Loop through existing children and update or remove as needed!
  ...
  for (let i = processInputs.children.length; i < numProcesses; i++) {
      ...
      nameInput.value = `P${i + 1}`; // Pre-load with P1, P2, P3, basically the process name with the index
...
function solve() {
  ...
  for (let i = 0; i < numProcesses; i++) {
      ...
      const priority = algorithm === 'Priority' ? parseInt(document.getElementById(`priority${i}`).value) : 0;

      if (isNaN(burst) || isNaN(arrival) || (algorithm === 'Priority' && isNaN(priority)) || !name) {
  ...
  if (algorithm === 'FCFS') {
      ...
      while (completedProcesses < numProcesses) {
          ...
          if (process) {
              ganttChart.push({ process: process.id, start: time, end: time + process.burst });
              time += process.burst;
  ...
  } else if (algorithm === 'RR') { // Don't know how this works, it is magic as far as I'm concerned, do not make any breaking changes without a save!
...
function displayGanttChart(ganttChart) {
  ...
  ganttChart.forEach((block, index) => {
      ...
      ganttBar.style.backgroundColor = block.process === 'IDLE' ? 'red' : (index % 2 === 0 ? '#007bff' : '#0054ad');
